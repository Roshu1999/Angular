...
namespace UIDesign.ServicesCSharp
{
    public class APIS
    {
        ...
        public APIS(IHttpContextAccessor contextAccessor)
        {
        ...
        public async Task<int> getcartitemcount(string username)
        {
            using (HttpClient client = new HttpClient())
            {
                ...
                client.BaseAddress = new Uri("http://fake.alpha.com/");     //APIGateway_BaseAddress);
                client.DefaultRequestHeaders.Accept.Clear();
                //string serializedObject = JsonConvert.SerializeObject(model);
                //HttpContent contentPost = new StringContent(serializedObject, Encoding.UTF8, "application/json");
                HttpResponseMessage response =  await client.GetAsync("api/Cart/username?username=" + username);
                //****************************************************************************************************************************** 
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    return catitems.Sum(c => c.quantity);
                }
